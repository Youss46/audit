-- ============================================================
-- MIGRATION DE PRODUCTION — M15 AUDIT
-- Script sécurisé (IF NOT EXISTS / ADD COLUMN IF NOT EXISTS)
-- À exécuter UNE FOIS sur la base PostgreSQL de production.
-- ============================================================
...
-- users : station_id, associated_cash_account_number,
--         requires_password_change, temporary_password_plain
ALTER TABLE "users"
...
-- transactions : station_id, supplier_name, supplier_ncc, invoice_number,
...
-- clients : nouveaux champs de configuration SYSCOHADA
ALTER TABLE "clients"
...
-- cash_registers : syscohada_account, owner_user_id (si manquants)
ALTER TABLE "cash_registers"
  ADD COLUMN IF NOT EXISTS "syscohada_account" text,
  ADD COLUMN IF NOT EXISTS "owner_user_id" integer;

-- fixed_assets : synced_from_transaction_id (si manquant)
ALTER TABLE "fixed_assets"
  ADD COLUMN IF NOT EXISTS "synced_from_transaction_id" integer;

-- documents : is_archived, fiscal_year, folder_category (si manquants)
...
-- document_folders : is_archived, fiscal_year, folder_category, created_by_id (si manquants)
...
-- missions : visa_stamp_code, visa_issued_at (si manquants)
...
-- ─── 3. Clés étrangères (safe: ignore si déjà présentes) ─────────────────────
...
-- ─── 4. Index (IF NOT EXISTS) ─────────────────────────────────────────────────

CREATE INDEX IF NOT EXISTS "transactions_station_id_idx" ON "transactions" USING btree ("station_id");

COMMIT;

-- ─── 5. Marquer la migration dans drizzle.__drizzle_migrations ───────────────
-- (empêche le migrateur de startup de re-exécuter le schema initial)
...
SELECT 'f107e607a38332f386706817df820c144fba95ade83a2682d5edfffaa502c47f',
       extract(epoch from now())::bigint * 1000
WHERE NOT EXISTS (
  SELECT 1 FROM drizzle.__drizzle_migrations
  WHERE hash = 'f107e607a38332f386706817df820c144fba95ade83a2682d5edfffaa502c47f'
);
